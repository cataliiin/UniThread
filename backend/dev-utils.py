...
db_app = typer.Typer(
    help='Gestionare Baza de Date [italic]("db --help" sub-comenzi)[/italic]',
    add_completion=False,
)
st_app = typer.Typer(
    help='Gestionare Stocare [italic]("storage --help" sub-comenzi)[/italic]',
    add_completion=False,
)
...
def db_add_university(
    ...
    async def _op(conn):
        try:
            ...
            console.print(
                f"[bold green][SUCCESS] Universitatea '{name}' ({domain}) a fost adaugata.[/bold green]"
            )
        except Exception as e:
            console.print(
                f"[bold red][ERROR] Eroare la adaugarea universitatii (posibil duplicat): {e}[/bold red]"
            )
            raise typer.Exit(1)
...
def reset_all(force: bool = typer.Option(False, "--force", "-f", help="Skip confirm")):
    ...
    console.print(
        Panel("[bold red][NUCLEAR RESET] UniThread Ecosystem[/bold red]", expand=False)
    )
