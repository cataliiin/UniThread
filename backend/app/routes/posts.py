...
async def get_global_feed(current_user: CurrentUser, db: DbDep, page: int = 1, size: int = 20, sort: str = "new"):
    """
...
async def create_post(post_in: PostCreate, current_user: CurrentUser, db: DbDep):
    ...
    member = await db.scalar(
        ...
        .where((CommunityMember.community_id == post_in.community_id) & (CommunityMember.user_id == current_user.id))
    )
...
async def update_post(post_id: UUID, post_in: PostUpdate, current_user: CurrentUser, db: DbDep):
    """
...
async def vote_post(post_id: UUID, vote_in: VoteCreate, current_user: CurrentUser, db: DbDep):
    """
